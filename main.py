...
class Camera:
    def __init__(self, lat, lon , alt, roll, pitch, yaw, px, py, f, m_a, m_b):
        self.lat = lat # latitude(metres)
        self.lon = lon # longitude(metres)
    ...
    def get_coordinates_ideal(self):
        return round(self.lat + self.get_x_position_ideal(), for_round), round(self.lon + self.get_y_position_ideal(), for_round), self.alt
